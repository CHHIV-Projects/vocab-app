...
class SynonymTests(unittest.TestCase):
    def test_synonyms_are_normalized_unique_and_limited(self):
        synsets = [
            FakeSynset("bright", "bright_light", "target"),
            FakeSynset("bright", "clear", "vivid", "radiant", "shining"),
        ]
        ...
        self.assertIn("bright light", result)
        self.assertEqual(len(result), 5)
        self.assertEqual(len({value.lower() for value in result}), len(result))
